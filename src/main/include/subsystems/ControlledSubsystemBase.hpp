        ...
        if (IsTesting()) {
            if (!frc::SetCurrentThreadPriority(true, kPrioLoggingThread)) {
                throw std::runtime_error(fmt::format(
                    "Setting logging thread RT priority to {} failed\n",
                    kPrioLoggingThread));
            }

            // Loops until the thread is told to exit and all remaining queue
            // items have been written to the logs
            while (m_entryThreadRunning || !m_entryQueue.empty()) {
                ...
                m_csvLogger.Log(
                    entry.nowBegin - frc::CSVLogFile::GetStartTime(), entry.r,
                    entry.x, entry.u, entry.y);
